...
def train(
    ...
    if plot:
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig(f'./Figure/{args.dataset}-{datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")}.png' ,dpi=300, bbox_inches='tight')
        plt.close()

    model.load_state_dict(best_state_dict)
    train_mae, train_mape, train_rmse, = MAE_MAPE_RMSE(*predict(model, trainset_loader))
    val_mae, val_mape, val_rmse = MAE_MAPE_RMSE(*predict(model, valset_loader))
...
def test_model(model, testset_loader, log=None):
    ...
    test_value = {"y_pred": y_pred, "y_true":y_true}
    np.savez("test_value.npz", **test_value)

    mae_all, mape_all, rmse_all = MAE_MAPE_RMSE(y_true, y_pred)
    ...
    for i in range(out_steps):
        mae, mape, rmse = MAE_MAPE_RMSE(y_true[:, i, :], y_pred[:, i, :])
...
if __name__ == "__main__":
    ...
    model_name = TrafficModel.__name__
    ...
    (trainset_loader, valset_loader, testset_loader, SCALER, adj_mx) = (
        get_dataloader_from_index_data(
            data_path,
    ...
    criterion = MaskedHuberLoss()  # MaskedMAELoss(), MaskedHuberLoss
    ...
    save_path = f"../saved_models/{model_name}-{dataset}"
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    if args.mode == "train":
        save = os.path.join(save_path, f"{model_name}-{dataset}-{now}.pt")
        shutil.copy2(f'{model_name}.py', save_path)
    elif args.mode == "test":
        # save_path = f"../saved_models/{model_name}-{dataset}"

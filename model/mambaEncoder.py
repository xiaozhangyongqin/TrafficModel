...
class MambaConfig:
    ...
    expand_factor: float = 2.0 # E in paper/comments
    d_conv: int = 4
    dropout: float = 0.2
    ...
    def __post_init__(self):
        self.d_inner = self.d_model // 2  # E*D = ED in comments
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.out_proj = nn.Linear(config.d_inner * 2, config.d_model, bias=config.bias)
        ...
        self.tcn = TCN(
            ...
            output_size=config.d_inner,
            num_channels=[config.d_inner], 
            kernel_size=3, 
    ...
    def forward(self, x):
        ...
        # z branch
        # z = F.silu(z)
        z = F.silu(self.tcn_z(z))

        output = torch.cat([y, z], dim=-1) # (B, L, 2*ED)
        # output = y * z
